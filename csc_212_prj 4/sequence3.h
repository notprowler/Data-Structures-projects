...
namespace main_savitch_5
{
    ...
    public:
        ...
        sequence( ); // O(1)
        sequence(const sequence& source); //O(n)
        ~sequence( ); //O(n)
        ...
        void operator =(const sequence& source); //O(n)
        void remove_current( ); //O(1)
        // CONSTANT MEMBER FUNCTIONS
        size_type size( ) const { return many_nodes; } //O(1)
        bool is_item( ) const { return (cursor != NULL); } //O(1)
        value_type current( ) const { return cursor->data(); } //O(1)
    private:
        node *head_ptr;
        node *tail_ptr;
        node *cursor;
